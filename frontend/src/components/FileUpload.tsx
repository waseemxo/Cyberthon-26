    ...
    <div className="w-full max-w-2xl mx-auto">
      <div
        ...
        className={`relative border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer group ${
          isDragActive
            ? 'border-primary-light bg-primary/10 scale-[1.02]'
            : isAnalyzing
      ...
      >
        ...
        {isAnalyzing ? (
          <div className="animate-fade-in">
            <Loader2 className="w-12 h-12 text-primary-light mx-auto mb-4 animate-spin" />
            <p className="text-lg font-medium text-text-primary mb-2">
              Analyzing content...
            </p>
            {/* Progress bar */}
            <div className="w-full max-w-xs mx-auto bg-surface-lighter rounded-full h-2 overflow-hidden">
              <div
          ...
          <div className="animate-fade-in">
            <div
              className={`w-16 h-16 rounded-2xl ${FILE_TYPE_COLORS[fileType]} flex items-center justify-center mx-auto mb-4`}
            >
          ...
          <>
            <div className="w-16 h-16 rounded-2xl bg-surface-lighter flex items-center justify-center mx-auto mb-4 group-hover:bg-primary/10 transition-colors">
              <Upload className="w-8 h-8 text-text-muted group-hover:text-primary-light transition-colors" />
            </div>
            <p className="text-lg font-medium text-text-primary mb-2">
              {isDragActive ? 'Drop file here' : 'Drop a file or click to upload'}
            </p>
            ...
            <div className="flex items-center justify-center gap-3 mt-6">
              {(['text', 'image', 'audio', 'video'] as FileType[]).map((type) => {
                ...
                return (
                  <div
                    key={type}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg ${FILE_TYPE_COLORS[type]} text-xs font-medium`}
                  >
